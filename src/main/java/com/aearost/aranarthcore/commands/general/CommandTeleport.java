...
public class CommandTeleport implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
		if (sender instanceof Player player) {
			if (args.length == 0) {
				sender.sendMessage(ChatUtils.chatMessage(Lang.get("general.invalid_syntax", "usage", "tp <player>")));
				return true;
			} else {
				...
				if (targetUUID != null) {
					...
					if (target != null) {
						...
						player.sendMessage(ChatUtils.chatMessage(Lang.get("teleport.request_sent", "player", targetPlayer.getNickname())));
						target.sendMessage(ChatUtils.chatMessage(Lang.get("teleport.request_received", "player", senderPlayer.getNickname())));
						...
						AranarthPlayer senderPlayer = AranarthUtils.getPlayer(player.getUniqueId());
						player.sendMessage(ChatUtils.chatMessage(Lang.get("teleport.request_sent", "player", remoteTarget.getNickname())));
