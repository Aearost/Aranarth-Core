		...
		if (sender instanceof Player player) {
			...
			if (args.length == 0) {
				...
				if (args[0].equalsIgnoreCase("create")) {
					...
					if (args.length < 2) {
						player.sendMessage(ChatUtils.chatMessage("&cInvalid syntax: &e/shop create <username>"));
						return true;
					}
					...
					if (uuid != null) {
						if (AranarthUtils.getShopLocations().containsKey(uuid)) {
							player.sendMessage(ChatUtils.chatMessage("&e" + args[1] + " &calready has a shop location!"));
							return true;
						}
						...
						player.sendMessage(ChatUtils.chatMessage("&7A shop has been created for &e" + args[1]));
					} else {
					...
					if (uuid != null) {
						if (AranarthUtils.getShopLocations().containsKey(uuid)) {
							...
							player.sendMessage(ChatUtils.chatMessage("&e" + args[1] + "'s &7shop has been deleted"));
						} else {
							player.sendMessage(ChatUtils.chatMessage("&e" + args[1] + " &cdoes not have a shop"));
						}
					} else {
						player.sendMessage(ChatUtils.chatMessage("&e" + args[1] + " &ccould not be found!"));
					}
