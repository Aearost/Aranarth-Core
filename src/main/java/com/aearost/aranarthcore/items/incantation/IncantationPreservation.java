...
public class IncantationPreservation implements Incantation {

    @Override
    public ItemStack getItem() {
        ItemStack incantation = new ItemStack(Material.PURPLE_DYE, 1);

        ItemMeta meta = incantation.getItemMeta();
        if (Objects.nonNull(meta)) {
            NamespacedKey key = new NamespacedKey(AranarthCore.getInstance(), "incantation_preservation");
            meta.setItemModel(key);
            meta.getPersistentDataContainer().set(INCANTATION_TYPE, PersistentDataType.STRING, "incantation_preservation");
