...
public class RecipeSawmill {
	...
	private void registerAll(Plugin plugin) {
		...
		for (Object[] wood : woodTypes) {
			...
			Material sign      = (Material) wood[14];
			...
			add(plugin, planks.name().toLowerCase() + "_stick",      Material.STICK, 3, planks);
			add(plugin, planks.name().toLowerCase() + "_sign",       sign, 1, planks);
		}
		...
		add(plugin, "bamboo_planks_stick",          Material.STICK, 3, Material.BAMBOO_PLANKS);
		add(plugin, "bamboo_planks_sign",           Material.BAMBOO_SIGN, 1, Material.BAMBOO_PLANKS);
	}
