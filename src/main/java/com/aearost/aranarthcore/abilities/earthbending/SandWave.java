...
public class SandWave extends SandAbility implements AddonAbility {
    ...
    // t ∈ [0,1] where the peak begins and where the crash returns to baseline —
    // derived directly from the WAVE_PROFILE indices above (peak=index 9, baseline=index 16)
     ...
     *
     * <p><b>Back slope</b>: particles fill solid columns from ground to a height
     * that decreases linearly over {@code BACK_SLOPE_LENGTH} blocks behind the
     * face, giving the wave visual body and depth.
     *
     * <p><b>Front face</b>: a solid filled wall at the current wave front.
     * During the crash phase each height layer is shifted forward by
     * {@code currentLean × (h/H)²}, so the crest overhangs the base — exactly
     * like a breaking ocean wave.
     *
     * <p>Both passes taper parabolically across the width so the centre is
     * tallest and the edges fade to zero.  Ground Y is sampled once per depth
     * slice along the centre line and shared across the full width of that slice.
     */
    ...
    private void checkEntityCollisions(Location waveFront, double waveHeight) {
        ...
        for (Entity entity : waveFront.getWorld().getNearbyEntities(
            ...
            if (forwardDist < -hitDepth || forwardDist > hitDepth) continue;
            if (perpDist    > hitWidth)                             continue;
            if (heightDiff  < -0.5 || heightDiff > hitHeight)      continue;
    ...
    // -------------------------------------------------------------------------
    // Static helpers — pending source + indicator
    // -------------------------------------------------------------------------
    ...
    @Override public String   getName()             { return "SandWave"; }
    @Override public void     load()                {}
    @Override public void     stop()                {}
    @Override public String   getAuthor()           { return "Aearost"; }
    @Override public String   getVersion()          { return "1.0"; }

    @Override
    public String getDescription() {
        return "Raise a towering wave of sand that surges forward, building to a crest before crashing down " +
                "on everything in its path. Targets hit take damage, are blinded, and are knocked back.\n" +
                ChatUtils.translateToColor("&fUsage: Sneak to select a sandbendable source, then Left-Click to launch");
    }
}
