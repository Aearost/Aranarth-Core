...
public class PetHurtPrevent {
    ...
    /**
     * Returns true if the given player would be blocked from attacking the target.
     */
    public static boolean wouldBeBlocked(Player attacker, LivingEntity target) {
        if (!(target instanceof Tameable pet) || !pet.isTamed()) {
            return false;
        }
        if (!(pet.getOwner() instanceof OfflinePlayer owner)) {
            return false;
        }
        if (owner.getUniqueId().equals(attacker.getUniqueId())) {
            AranarthPlayer aranarthAttacker = AranarthUtils.getPlayer(attacker.getUniqueId());
            return !aranarthAttacker.isHurtingOwnPets();
        } else {
            Dominion chunkDominion = DominionUtils.getDominionOfChunk(target.getLocation().getChunk());
            return chunkDominion != null
                    && target instanceof Sittable sittable
                    && sittable.isSitting();
        }
    }
}
